...
indikatorSistem <- read.table("data/indikatorsistem.csv", header=FALSE, sep=",")
colnames(indikatorSistem)<-"Indikator"
result_Sistem <-cbind(indikatorSistem,tempSistem)
...
graphSistem<-cbind(indikatorSistem,t((val_Sistem[i,1:18])),t(val_Sistem[i,19:36]))
...
indikatorOrg <- read.table("data/indikatorOrganisasi.csv", header=FALSE, sep=",")
colnames(indikatorOrg)<-"Indikator"
result_Organisasi <-cbind(indikatorOrg,tempOrganisasi)
...
graphOrg<-cbind(indikatorOrg,t((val_Organisasi[i,1:15])),t(val_Organisasi[i,16:30]))
